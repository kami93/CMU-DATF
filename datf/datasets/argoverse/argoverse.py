...
def argoverse_collate(batch, test_set=False):
    ...
    if test_set:
        try:
            ...
            print("[ERROR] Check if you are using test.py for test_set or not, if its valid dataset, dont use test_set=True, Or else: ")
            print(e)
    else:
        try:
            ...
            print("[ERROR] Check if you are using train=False and no test_set=False for test.py, or train=True for train.py. Or else: ")
            print(e)
...
class ArgoverseDataset(Dataset):
    def __init__(self, cfg, data_partition, map_version, **kwargs):
        ...
        cache_file = kwargs.get("cache_file", cfg.cache_path)
        ...
        if self.intrinsic_rate % sampling_rate:
        ...
        if cache_file is None:
            # cache_dir = "./argo_{}_cache.pkl".format(self.data_partition)
            cache_file_name = "argo_{}_cache.pkl".format(self.data_partition)
            cache_file = os.path.join(cache_file, cache_file_name)
            if os.path.isfile(cache_file):
                print(f"[LOG] Dataset cache found at {cache_file}")
                self.load_cache(cache_file)
            else:
                print(f"[LOG] Making and saving cache file")
    ...
    def __getitem__(self, idx):
        ...
        if self.use_scene:
            ...
            if self.map_version == '1.3':
                ...
                try:
                    raw_map_image = cv2.resize(raw_image.astype(np.float32), dsize=tuple(self.scene_size), interpolation=cv2.INTER_LINEAR)
                except Exception as e:
                    print(e)
